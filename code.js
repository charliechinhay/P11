...
/*-capire se un termine è contenuto in una stringa
Includes
-Rendere la ricerca caseInsensitive
*/
let titolo = "cus";
let zona = "us";

function searchJobs(titolo, zona) {
  let result = [];
  for (let i = 0; i < jobs.length; i++) {
    let offerta = jobs[i];
    let titoloMinuscolo = titolo.toLowerCase();
    let zonaMinuscolo = zona.toLocaleLowerCase();
    let titleLowerCase = offerta.title.toLowerCase();
    let offertaLowerCase = offerta.location.toLowerCase();
    if (
      titleLowerCase.includes(titoloMinuscolo) &&
      offertaLowerCase.includes(zonaMinuscolo)
    ) {
      result.push(offerta);
    }
  }
